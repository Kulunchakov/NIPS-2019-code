...
template <> inline bool mexCheckType<unsigned char>(const mxArray* array) {
...
template <typename T> inline void getVector(const mxArray* array, Vector<T>& y) {
   if (!mexCheckType<T>(array)) 
      mexErrMsgTxt("type of vector is not consistent");
   if (mxIsSparse(array))
...
template <typename T> inline void getMatrix(const mxArray* array, Matrix<T>& X) {
   if (!mexCheckType<T>(array)) 
      mexErrMsgTxt("type of matrix is not consistent");
   if (mxIsSparse(array))
      mexErrMsgTxt("argument should not be sparse");
   const mwSize* dims=mxGetDimensions(array);
...
template <typename T> inline void getMatrix_debug(const mxArray* array, Matrix<T>& X, const string& name, const int ii) {
   if (!mexCheckType<T>(array)) {
      std::cerr << "Warning with " << name << " " << ii << std::endl;
      mexErrMsgTxt("type of matrix is not consistent");
   }
   if (mxIsSparse(array))
      mexErrMsgTxt("argument should not be sparse");
...
template <typename T> inline void getMap(const mxArray* array, Map<T>& X) {
   if (!mexCheckType<T>(array)) 
      mexErrMsgTxt("type of map is not consistent");
   if (mxIsSparse(array))
      mexErrMsgTxt("argument should not be sparse");
   const mwSize* dims=mxGetDimensions(array);
   const mwSize num_dims=mxGetNumberOfDimensions(array);
   INTM m=static_cast<INTM>(dims[0]);
   INTM n=static_cast<INTM>(dims[1]);
   INTM z=num_dims ==2 ? 1 : static_cast<long long>(dims[2]);
   T* prX = reinterpret_cast<T*>(mxGetPr(array));
   X.setData(prX,m,n,z);
};
