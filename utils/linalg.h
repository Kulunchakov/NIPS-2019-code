...
template<typename T> class CudaVector {
   ...
   public:
      ...
      inline void div(const CudaVector<T>& x);
...
template<typename T> inline void CudaVector<T>::getVector(Vector<T>& vec) const {
   vec.resize(_n);
   stat = cublasGetVector (_n, sizeof(T), _X,1, vec.rawX(), 1);
};

template<typename T> inline void CudaVector<T>::setVector(const Vector<T>& vec) {
   resize(vec.n());
   stat = cublasSetVector (_n, sizeof(T), vec.rawX(), 1, _X, 1);
};
...
template <typename T> inline void CudaVector<T>::div(const CudaVector<T>& vec) {
   cuda_div(_n,_X,vec._X);
};
...
template<typename T> class CudaMatrix {
   ...
   public:
      ...
      inline void scal(const T a);
      inline T asum() const;
      inline void eigSym(CudaMatrix<T>& U, CudaVector<T>& S, const bool cpu = false) const;
      ...
      inline void mult_elementWise(const CudaMatrix<T>& B, CudaMatrix<T>& C);
...
template<typename T> inline void CudaMatrix<T>::getMatrix(Matrix<T>& mat) const {
   mat.resize(_m,_n);
   stat = cublasGetMatrix (_m, _n, sizeof(T), _X, _m, mat.rawX(), _m);
};

template<typename T> inline void CudaMatrix<T>::setMatrix(const Matrix<T>& mat) {
   resize(mat.m(),mat.n());
   stat = cublasSetMatrix (_m, _n, sizeof(T), mat.rawX(), _m, _X, _m);
};
...
template <typename T> inline void CudaMatrix<T>::mult_elementWise(const CudaMatrix<T>& B, CudaMatrix<T>& C) {
   C.resize(_m,_n);
   cuda_mult(_m*_n,_X,B._X,C._X);
}
...
template <typename T> inline T CudaMatrix<T>::asum() const {
   return cublas_asum(_m*_m,_X,1);
}
...
template <typename T> inline T CudaMatrix<T>::normFsq() const {
   return cublas_dot(_m*_n,_X,1,_X,1);
};
...
template <> inline void CudaMatrix<double>::mult(const CudaMatrix<double>& B, CudaMatrix<double>& C, 
   ...
   C.resize(m,n);
   stat=cublasDgemm(handle,trA, trB,m,n,k, &a,_X,_m,B._X,B._m,&b,C._X,C._m);
};
...
template <> inline void CudaMatrix<float>::mult(const CudaMatrix<float>& B, CudaMatrix<float>& C, 
   ...
   //PRINT_I(n)
   C.resize(m,n);
   stat=cublasSgemm(handle,trA, trB,m,n,k, &a,_X,_m,B._X,B._m,&b,C._X,C._m);
};
...
template <> 
inline void CudaMatrix<float>::eigSym(CudaMatrix<float>& U, CudaVector<float>& S, const bool cpu) const {
#ifdef CUDA_8
   if (!cpu) {
      U.resize(_m,_m);
      S.resize(_m);
...
template <> 
inline void CudaMatrix<double>::eigSym(CudaMatrix<double>& U, CudaVector<double>& S, const bool cpu) const {
#ifdef CUDA_8
   if (!cpu) {
      U.resize(_m,_m);
      S.resize(_m);
